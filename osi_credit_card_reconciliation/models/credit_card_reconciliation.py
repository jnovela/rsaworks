...
class CcRecStatement(models.Model):
    ...
    def check_group(self):
        ...
        CC Statement Preparer– they can create, view and delete any of the Bank Statements provided the Bank Statement is not in the DONE state,
        ...
        group_verifier_id = self.env['ir.model.data'].xmlid_to_res_id('osi_credit_card_reconciliation.group_cc_stmt_verifier')
        for statement in self:
            if group_verifier_id:
                # Browse id on res.groups
                group_verifier = res_groups_obj.browse(group_verifier_id)
                group_user_ids = [user.id for user in group_verifier.users]
                if statement.state!='draft' and self._uid not in group_user_ids:
                    raise UserError(_("Only a member of '%s' group may delete/edit cc statements when not in draft state!" %(group_verifier.name)))
        return True
    ...
    def copy(self, default={}):
        ...
        return super(CcRecStatement, self).copy(default)
    ...
    def write(self, vals):
        ...
        if 'journal_id' in vals:
            ...
            if journal and journal.partner_id and journal.partner_id.property_account_payable_id:
                vals.update({'account_id': journal.partner_id.property_account_payable_id.id})
        return super(CcRecStatement, self).write(vals)
    ...
    def unlink(self):
        ...
        statement_line_obj = self.env['cc.rec.statement.line']
        self.check_group() # Check if the user is allowed to perform the action
        for statement in self:
        ...
        return super(CcRecStatement, self).unlink()
        ...
        for statement in self:
            if statement.cleared_balance_cur:
                if statement.difference_cur != 0.0:
                    raise Warning(_('Prior to reconciling a statement, all differences must be accounted for and the Difference must be zero." \
    ...
    def action_process(self):
        """Set the account move lines as 'Cleared' and Assign 'CC Acc Rec Statement ID'
        ...
        for statement in self:
            ...
            for statement_line in statement_lines:
                #Mark the move lines as 'Cleared'mand assign the 'CC Acc Rec Statement ID'
                statement_line.move_line_id.write({'cleared_cc_account': statement_line.cleared_cc_account,
                                             'cc_rec_statement_id': statement_line.cleared_cc_account and statement.id or False
                                             })
    ...
    def action_cancel_draft(self):
        ...
        statement_line_ids = self.env['cc.rec.statement.line']
        for statement in self:
            statement_lines = statement.credit_move_line_ids + statement.debit_move_line_ids
            for statement_line in statement_lines:
                ...
                if statement_line.move_line_id:
                    line_ids |= statement_line.move_line_id # Find move lines related to statement lines
            ...
            statement_line_ids.write({'cleared_cc_account': False,
    ...
    def action_select_all(self):
        ...
        for statement in self:
            ...
            statement_line_ids.write({'cleared_cc_account': True})
        return True
    ...
    def refresh_record(self):
        ...
        for statement in self:
            ...
            if statement.state == 'draft':
                ...
                journal_id = statement.journal_id and statement.journal_id.id
                ...
                for cr_item in vals['value']['credit_move_line_ids']:
                    cr_item['cleared_cc_account'] = refdict and refdict.get(cr_item['move_line_id'], False) or False
                ...
                for dr_item in vals['value']['debit_move_line_ids']:
                    dr_item['cleared_cc_account'] = refdict and refdict.get(dr_item['move_line_id'], False) or False
                ...
                retval = self.write({'last_ending_date':vals['value']['last_ending_date'],
    ...
    def onchange_journal_id(self):
        ...
        if self.journal_id:
            if not self.journal_id.partner_id:
                raise Warning(_('Please define credit card company to the selected journal (%s).' % self.journal_id.name))
...
#                 statement_line_obj.unlink(statement_line_ids)
                ...
                #2. Journal items which are not cleared in previous cc statements
                ...
                for line in line_ids:
                    ...
                    counterpart_lines = line.move_id.line_ids.filtered(lambda ids: ids.account_id.id == ids.journal_id.default_debit_account_id.id)
                ...
                prev_stmt=self.get_starting_balance(journal_id, account_id, ending_date)
    ...
    name = fields.Char('Name', required=True, size=64, states={'done':[('readonly', True)]},
                        help="This is a unique name identifying the statement (e.g. Credit Card X January 2012).")
    account_id = fields.Many2one('account.account', 'Account',
                                 states={'done':[('readonly', True)]},
                                 domain="[('company_id', '=', company_id), ('type', '!=', 'view')]",
                                 help="The Credit Card Payable Account that is being reconciled.")
    ending_date = fields.Date('Statement Date', required=True,
                              states={'done':[('readonly', True)]},
                              default=lambda *a: time.strftime('%Y-%m-%d'),
                              help="Statement Date")
    last_ending_date = fields.Date('Last Stmt Date',
                                    help="The previous statement date of your credit crd statement.")
    starting_balance = fields.Float('Starting Balance (b/fwd)', required=True,
                                     digits=dp.get_precision('Account'),
                                     help="Starting Balance for the current credit card statement.",
                                     states={'done':[('readonly', True)]})
    ending_balance = fields.Float('Statement Balance (due)', required=True,
                                  digits=dp.get_precision('Account'),
    ...
    verified_date = fields.Date('Verified Date',
                                states={'done':[('readonly', True)]},
                                help="Date in which Credit Card statement was verified.")
    ...
    credit_move_line_ids = fields.One2many('cc.rec.statement.line',
                                           'statement_id', 'Credits',
                                            domain=[('type','=','cr')],
                                            context={'default_type':'cr'},
                                            states={'done':[('readonly', True)]})
    debit_move_line_ids = fields.One2many('cc.rec.statement.line',
                                          'statement_id', 'Debits',
                                           domain=[('type','=','dr')],
    ...
    sum_of_ucredits_lines = fields.Float(compute='_get_balance',
                                         ...
                                         help="Credit Card Charges, Fees, Interest and Penalties # of Items")
    suppress_ending_date_filter = fields.Boolean('Show all charges, credits and payments',
    ...
    def _get_balance(self):
        ...
        A) Credit card payments/credits Amount: Total SUM of Amts of lines with Cleared = True
        Credit card payments/credits # of Items: Total of number of lines with Cleared = True
        ...
        for statement in self:
            ...
            for line in statement.credit_move_line_ids:
                statement.sum_of_credits += line.cleared_cc_account and round(line.amount, account_precision) or 0.0
                statement.sum_of_credits_cur += line.cleared_cc_account and round(line.amountcur, account_precision) or 0.0
                statement.sum_of_credits_lines += line.cleared_cc_account and 1.0 or 0.0
                statement.sum_of_ucredits += (not line.cleared_cc_account) and round(line.amount, account_precision) or 0.0
                statement.sum_of_ucredits_cur += (not line.cleared_cc_account) and round(line.amountcur, account_precision) or 0.0
                statement.sum_of_ucredits_lines += (not line.cleared_cc_account) and 1.0 or 0.0
            for line in statement.debit_move_line_ids:
                statement.sum_of_debits += line.cleared_cc_account and round(line.amount, account_precision) or 0.0
                statement.sum_of_debits_cur += line.cleared_cc_account and round(line.amountcur, account_precision) or 0.0
                statement.sum_of_debits_lines += line.cleared_cc_account and 1.0 or 0.0
                statement.sum_of_udebits += (not line.cleared_cc_account) and round(line.amount, account_precision) or 0.0
                statement.sum_of_udebits_cur += (not line.cleared_cc_account) and round(line.amountcur, account_precision) or 0.0
                statement.sum_of_udebits_lines += (not line.cleared_cc_account) and 1.0 or 0.0
 
            statement.cleared_balance = abs(round(statement.sum_of_debits - statement.sum_of_credits, account_precision))
            statement.cleared_balance_cur = abs(round(statement.sum_of_debits_cur - statement.sum_of_credits_cur, account_precision))
            statement.uncleared_balance = abs(round(statement.sum_of_udebits - statement.sum_of_ucredits, account_precision))
            statement.uncleared_balance_cur = abs(round(statement.sum_of_udebits_cur - statement.sum_of_ucredits_cur, account_precision))
            statement.difference = round((abs(statement.starting_balance - statement.ending_balance) - statement.cleared_balance), account_precision)
            statement.difference_cur = round((abs(statement.starting_balance - statement.ending_balance) - statement.cleared_balance_cur), account_precision)
...
class CCRecStatementLine(models.Model):
    ...
    amountcur = fields.Float(string='Amount in Currency', digits=dp.get_precision('Account'),
    ...
    statement_id = fields.Many2one('cc.rec.statement', 'Statement', required=True, ondelete='cascade')
    move_line_id = fields.Many2one('account.move.line', 'Journal Item',
                                   help="Related Journal Item.")
    cleared_cc_account = fields.Boolean('CC Cleared? ',
    ...
    def create(self, vals):
        ...
        if 'from_filter' in vals and vals['from_filter'] == False:
            raise Warning(
                        _('You cannot add any new bank statement line\
                         manually as of this revision for Name "%s" !') % vals['name'])
        return super(CCRecStatementLine, self).create(vals)
    ...
    def unlink(self):
        for x in self:
            ...
            if x.move_line_id:
                x.move_line_id.write({'cleared_cc_account': False,
                                     'cc_rec_statement_id': False,
                                     })
        return super(CCRecStatementLine, self).unlink()
